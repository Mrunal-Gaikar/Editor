...
#########################################################################
def about():
    mb.showinfo("About", "A Editor using Tkinter by SCTPL")

def help_box(event=None):
    mb.showinfo("Help", "For help email to rockyjagtiani@gmail.com", icon='question')
...
def new_file(event=None):
    global filename        #can be accessed by har ek function 
    filename = None
    root.title("Newdoc-Tkeditor")
...
def open_file(event=None):
    global filename
    filename =fd.askopenfilename(defaultextension=".txt",filetypes=[("All Files","*.*"),("Text Documents","*.txt"),("Python File","*.py")])
    if filename == "": # If no file chosen.
        
        f = open("Untitled_copy.txt", 'r')
        fh=open("Untitled_copy.txt","w+")
        ...
        root.title(os.path.basename(filename) + " - Tkeditor") # Returning the basename of 'file'  os.path gives whole path
        textPad.delete(1.0,END)         
        fh = open(filename,"r")   
        f = open(filename+"copy.txt", 'w+')     
...
def save_as():
    try:
        ...
        f = fd.asksaveasfilename(initialfile='Untitled.txt',defaultextension=".txt",filetypes=[("All Files","*.*"),("Text Documents","*.txt"),("Python File","*.py")])
        fh = open(f, 'w')           
        global filename
        filename = f
...
# PhotoImage is converting files into object n saving as newicon
...
#File menu    
filemenu = Menu(menubar, tearoff=1) #tearoff to tear file menu
...
#Checking is file is present at this path or not
if(os.path.isfile('C:/Users/home/workspace/project_day/Untitled_copy.txt')):
    fh = open("Untitled_copy.txt","r+")   
    textPad.insert(1.0,fh.read())
else:
    fh=open("Untitled_copy.txt","w+")
